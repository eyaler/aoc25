...
total = 0
for row in rows:
    target, *buttons, joltage = row.split()
    target = [c == '#' for c in target[1:-1]]
    buttons = [list(map(int, button[1:-1].split(','))) for button in buttons]
    buttons = [[i in button for i in range(len(target))] for button in buttons]
    joltage = [int(i) for i in joltage[1:-1].split(',')]
    model = cp_model.CpModel()
    clicks = []
    for i in range(len(buttons)):
        clicks.append(model.new_int_var(0, max(joltage), f'click_{i}'))
    for i, j in enumerate(joltage):
        model.add(j == sum(click for click, button in zip(clicks, buttons) if button[i]))
    model.minimize(sum(clicks))
    solver = cp_model.CpSolver()
    solver.solve(model)
    steps = solver.value(sum(clicks))
    print(steps)
    total += steps
print(total)
