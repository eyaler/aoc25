...
def dfs(start, end):
    ...
    if (start, end) not in cache:
        cache[(start, end)] = sum(dfs(node, end) for node in graph[start])
    return cache[(start, end)]


print(dfs('svr', 'dac')*dfs('dac', 'fft')*dfs('fft', 'out') + dfs('svr', 'fft')*dfs('fft', 'dac')*dfs('dac', 'out'))
